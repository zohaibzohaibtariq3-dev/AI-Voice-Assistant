...
-----------------------------------
Pipeline: Browser mic audio -> Whisper (STT) -> Groq LLM -> gTTS (TTS) -> Browser playback

Endpoints:
    GET  /                 -> renders the UI
    POST /api/transcribe   -> audio blob  -> transcribed text
    POST /api/chat         -> text        -> LLM response text
    POST /api/speak        -> text        -> mp3 audio file
    POST /api/converse     -> audio blob  -> {user_text, ai_text, audio_url}  (full pipeline in one call)
"""
